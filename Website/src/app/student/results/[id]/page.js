...
export default function ResultDetails() {
    ...
    const predictedLevel = mlSummary?.predicted_level || result.ml_predicted_level || 'medium';
    const levelCfg = LEVEL_CONFIG[predictedLevel] || LEVEL_CONFIG.medium;
    const mlScore = mlSummary?.ml_score ?? result.ml_score ?? null;
                ...
                <div className="max-w-4xl mx-auto py-6 px-4 sm:px-6 lg:px-8">
                                        ...
                                        <div>
                                            ...
                                            <p className="font-semibold">
                                                {result.detailed_answers ?
                                                    result.detailed_answers.filter(a => a.is_correct).length : 0
                    ...
                    <div className="bg-white rounded-lg shadow-lg overflow-hidden mb-6">
                        ...
                        <div className="p-6 space-y-6">
                                    ...
                                    <div className="border border-purple-100 rounded-lg p-4">
                                        <div className="flex items-center justify-between mb-2">
                                            <span className="text-sm font-medium text-gray-600 flex items-center gap-1">
                                                <Zap className="h-4 w-4 text-purple-500" /> Adaptive Score
                                            </span>
                                            <span className="text-lg font-bold text-purple-700">
                                                {(mlScore * 100).toFixed(1)} / 100
                                            </span>
                                        </div>
                                        <div className="w-full bg-gray-200 rounded-full h-2.5">
                                            <div
                                                className="h-2.5 rounded-full bg-purple-500 transition-all duration-700"
                                                style={{ width: `${Math.round(mlScore * 100)}%` }}
                                            />
                                        </div>
                                        <p className="text-xs text-gray-500 mt-1">
                                            Formula: accuracy × 70% + speed × 30%
                                        </p>
                                    </div>
                                )}

                                {/* Recommended Level */}
                                <div className={`border ${levelCfg.border} ${levelCfg.bg} rounded-lg p-4`}>
                                    <div className="flex items-center gap-2 mb-1">
                                        <TrendingUp className={`h-4 w-4 ${levelCfg.color}`} />
                                        <span className="text-sm font-medium text-gray-600">Recommended Level</span>
                                    </div>
                                    <p className={`text-2xl font-bold capitalize ${levelCfg.color}`}>{levelCfg.label}</p>
                                    <p className="text-xs text-gray-500 mt-1">
                                        Based on your accuracy and response speed
                                    </p>
                                </div>
                            </div>
                            ...
                            {/* Weak Topics */}
                            {mlSummary?.wrong_topics?.length > 0 && (
                                <div>
                                    <h3 className="text-sm font-semibold text-gray-700 mb-2 flex items-center gap-1">
                                        <Target className="h-4 w-4 text-orange-500" /> Focus Areas
                                    </h3>
                                    <ul className="space-y-1">
                                        {mlSummary.wrong_topics.map((topic, i) => (
                                            <li key={i} className="text-sm text-gray-600 bg-orange-50 border border-orange-100 rounded px-3 py-1.5 flex items-start gap-2">
                    ...
                    {/* ── RECOMMENDATIONS ── */}
                    {recommendations && (
                        (recommendations.recommendations?.review?.length > 0 || recommendations.recommendations?.next?.length > 0) && (
                            <div className="bg-white rounded-lg shadow-lg overflow-hidden mb-6">
                                <div className="px-6 py-4 border-b border-green-100 bg-gradient-to-r from-green-50 to-emerald-50 flex items-center gap-2">
                                    <BookOpen className="h-5 w-5 text-green-600" />
                                    <h2 className="text-lg font-semibold text-green-900">Recommended for You</h2>
                                </div>

                                <div className="p-6 space-y-4">
                                    {recommendations.recommendations?.review?.length > 0 && (
                                        <div>
                                            <h3 className="text-sm font-semibold text-orange-700 mb-2 flex items-center gap-1">
                                                <Target className="h-4 w-4" /> Review — Weak Topics
                                            </h3>
                                            <div className="grid grid-cols-1 md:grid-cols-2 gap-3">
                                                {recommendations.recommendations.review.slice(0, 4).map(quiz => (
                                                    <Link
                                                        key={quiz.id}
                                                        href={`/student/quizzes/${quiz.id}`}
                                                        className="flex items-start gap-3 p-3 border border-orange-100 rounded-lg bg-orange-50 hover:bg-orange-100 transition-colors"
                                                    >
                                                        ...
                                                        <div>
                                                            ...
                                                            <p className="text-xs text-orange-600 mt-0.5">{quiz.reason}</p>
                                                        </div>
                                                    </Link>
                                                ))}
                                            </div>
                                    ...
                                    {recommendations.recommendations?.next?.length > 0 && (
                                        <div>
                                            <h3 className="text-sm font-semibold text-blue-700 mb-2 flex items-center gap-1">
                                                <TrendingUp className="h-4 w-4" /> Up Next — Your Level
                                            </h3>
                                            <div className="grid grid-cols-1 md:grid-cols-2 gap-3">
                                                {recommendations.recommendations.next.slice(0, 4).map(quiz => (
                                                    <Link
                                                        key={quiz.id}
                                                        href={`/student/quizzes/${quiz.id}`}
                                                        className="flex items-start gap-3 p-3 border border-blue-100 rounded-lg bg-blue-50 hover:bg-blue-100 transition-colors"
                                                    >
                                                        <Trophy className="h-5 w-5 text-blue-500 mt-0.5 flex-shrink-0" />
                                                        <div>
                                                            <p className="font-medium text-sm text-gray-900">{quiz.title}</p>
                                                            <p className="text-xs text-gray-500">{quiz.lesson_title}</p>
                                                            <p className="text-xs text-blue-600 mt-0.5">{quiz.reason}</p>
                                                        </div>
                                                    </Link>
                                                ))}
                                            </div>
